...
export default function CustomAutosuggest({
  ...
  const startValue = selected ? (toDisplayText ? toDisplayText(selected) : selected) : "";
  ...
  return (
    <Autosuggest
      ...
      getSuggestionValue={(suggestion) => {
        ...
        return toDisplayText ? toDisplayText(suggestion) : toSuggestionText ? toSuggestionText(suggestion) : suggestion;
      }}
      renderSuggestion={(suggestion, x) => {
        const text = toSuggestionText ? toSuggestionText(suggestion) : suggestion;
        if (!text) {
          return <span></span>;
        }
